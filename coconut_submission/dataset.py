...
# Fixed for transformers 4.35.0
def pad_without_fast_tokenizer_warning(batch, tokenizer, **kwargs):
    return batch
...
def get_dataset(path, tokenizer, max_size=1000000000, max_length=2048):
    ...
    def tokenize_sample(sample):
        ...
        total_length = len(question_tokenized) + sum(len(s) for s in steps_tokenized) + len(answer_tokenized)
        if total_length > max_length:
            # Strategy: Keep question and answer, truncate steps from the middle
            # This preserves the question context and final answer
            question_answer_length = len(question_tokenized) + len(answer_tokenized)
            available_for_steps = max_length - question_answer_length - 20  # 20 for safety margin
            
            if available_for_steps > 100:  # Only truncate if we have meaningful space
                # Truncate steps to fit
                truncated_steps = []
                current_length = 0
                for step in steps_tokenized:
                    if current_length + len(step) <= available_for_steps:
                        truncated_steps.append(step)
                        current_length += len(step)
                    else:
                        # Try to fit partial step
                        remaining = available_for_steps - current_length
                        if remaining > 50:  # Only if meaningful space
                            truncated_steps.append(step[:remaining])
                        break
                steps_tokenized = truncated_steps
            # If available_for_steps is too small, keep at least first and last step
            if len(steps_tokenized) == 0 and len(sample["steps"]) > 0:
                # Keep first step only
                steps_tokenized = [tokenizer.encode(sample["steps"][0] + "\n", add_special_tokens=False)]
